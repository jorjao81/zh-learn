...
class TestESpeakGenerator:
    """Test eSpeak-NG generator."""
    
    def test_availability_check(self):
        """Test eSpeak availability detection."""
        generator = ESpeakGenerator()
        
        # Mock subprocess to simulate eSpeak not available
        with patch('subprocess.run') as mock_run:
            mock_run.side_effect = FileNotFoundError()
            assert not generator.is_available()
        
        # Mock subprocess to simulate eSpeak available
        with patch('subprocess.run') as mock_run:
            mock_run.return_value.returncode = 0
            assert generator.is_available()
    
    def test_audio_generation_success(self):
        """Test successful audio generation with eSpeak."""
        with tempfile.TemporaryDirectory() as temp_dir:
            generator = ESpeakGenerator(temp_dir)
            output_file = os.path.join(temp_dir, "test.mp3")
            
            # Mock subprocess to simulate successful eSpeak execution
            with patch('subprocess.run') as mock_run:
                mock_run.return_value.returncode = 0
    ...
    def test_audio_generation_failure(self):
        """Test failed audio generation with eSpeak."""
        with tempfile.TemporaryDirectory() as temp_dir:
            generator = ESpeakGenerator(temp_dir)
            output_file = os.path.join(temp_dir, "test.mp3")
    ...
    def test_successful_audio_download(self, mock_get):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_response.json.return_value = {
                'items': [
                    {'pathmp3': 'http://example.com/audio.mp3', 'num_votes': 5}
                ]
            }
...
class TestWiktionaryGenerator:
    """Test Wiktionary generator."""
    
    def test_availability(self):
        """Test that Wiktionary is always available."""
        generator = WiktionaryGenerator()
        assert generator.is_available()
    
    @patch('requests.get')
    def test_successful_audio_extraction(self, mock_get):
        """Test successful audio extraction from Wiktionary."""
        with tempfile.TemporaryDirectory() as temp_dir:
            generator = WiktionaryGenerator(temp_dir)
            output_file = os.path.join(temp_dir, "test.mp3")
            
            # Mock HTML response with audio element
            html_content = '''
            <html>
                <source type="audio/ogg" src="//upload.wikimedia.org/wikipedia/commons/zh-test.ogg">
            </html>
            '''
            
            mock_page_response = Mock()
            mock_page_response.content = html_content.encode()
            mock_page_response.raise_for_status.return_value = None
            
            # Mock audio download
            mock_audio_response = Mock()
            mock_audio_response.content = b"fake ogg audio data"
            mock_audio_response.raise_for_status.return_value = None
            
            mock_get.side_effect = [mock_page_response, mock_audio_response]
            
            with patch('anki_pleco_importer.audio.BeautifulSoup') as mock_bs:
                mock_soup = Mock()
                mock_audio_element = Mock()
                mock_audio_element.get.return_value = "//upload.wikimedia.org/wikipedia/commons/zh-test.ogg"
                mock_soup.find_all.return_value = [mock_audio_element]
...
class TestAudioGeneratorFactory:
    ...
    def test_create_espeak_generator(self):
        """Test creating eSpeak generator."""
        generator = AudioGeneratorFactory.create_generator("espeak", {})
        assert isinstance(generator, ESpeakGenerator)
    ...
    def test_get_available_providers(self):
        ...
        config = {
            "espeak": {},
            "forvo": {"api_key": "test_key"},
            "azure": {}  # Missing required config
        }
        
        with patch.object(ESpeakGenerator, 'is_available', return_value=True), \
             patch.object(ForvoGenerator, 'is_available', return_value=True):
